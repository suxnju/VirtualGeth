// Ethereum address : 0xa8f9c7ff9f605f401bde6659fd18d9a0d0a802c5
// GroundTruth : 
/**
...
contract AccessControl is SafeMath{
    ...
    address public ceoAddress; // STORAGE[0x0]
    address public cfoAddress; // STORAGE[0x1]
    address public cooAddress; // STORAGE[0x2]

    address newContractAddress; // STORAGE[0x3]

    uint public tip_total = 0; // STORAGE[0x4]
    uint public tip_rate = 10000000000000000; // STORAGE[0x5]

    // @dev Keeps track whether the contract is paused. When that is true, most actions are blocked
    bool public paused = false; // STORAGE[0x6]
...
contract RpsGame is SafeMath , AccessControl{
    ...
    /// @dev struct of a game
    mapping (uint => mapping(uint => uint8)) public payoff; // STORAGE[0x7]
    mapping (uint => Game) public games; // STORAGE[0x8]
    mapping (address => uint[]) public gameidsOf; // STORAGE[0x9]

    /// @dev Current game maximum id
    uint public maxgame = 0; // STORAGE[0xa]
    uint public expireTimeLimit = 30 minutes; // STORAGE[0xb]
    ...
    function createGame(bytes32 dealerHash, address player) public payable whenNotPaused returns (uint){
        ...
        //     address dealer; //1
        //     uint dealerValue; //2
        //     bytes32 dealerHash; //3
        //     uint8 dealerChoice; //4
        //     address player; //4
        //     uint8 playerChoice; //4
        //     uint playerValue; //5
        //     uint8 result; //6
        //     bool closed; //6
        // }
        Game storage game = games[maxgame];
        game.dealer = msg.sender; //1
        game.player = player; //4
        game.dealerHash = dealerHash; //3
        
        game.dealerChoice = NONE; // 4

        game.dealerValue = msg.value; // 2
        game.expireTime = expireTimeLimit + now; //0
    ...
    function joinGame(uint gameid, uint8 choice) public payable whenNotPaused returns (uint){
        ...
        game.player = msg.sender; //4
        game.playerChoice = choice; //4
        game.playerValue = msg.value; //5
        game.expireTime = expireTimeLimit + now; //0
